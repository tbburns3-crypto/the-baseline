...
const GOLF_PAIRINGS_OVERRIDE = {
  '401811949': {
    ...
    groups: [
      ...
      ['T. Kim','Smotherman','B. Griffin'],
      ['Power','Eckroat','Kanaya'],
      ['M. Kim','Theegala','Moore'],
      ['Rooyen','Grillo','Roy'],
      ['Schenk','Riley','Fisk'],
      ['L. Griffin','Coody','Suber'],
      ['Garnett','Brennan','Ewart'],
      ['Woodland','Ghim','Aberg'],
      ['Hughes','Meissner','Hisatsune'],
      ['Henley','Bhatia','Smalley'],
      ['Novak','Neergaard-Petersen','Castillo'],
      ['Glover','Snedeker','Fishburn'],
      ['Thorbjornsen','Gerard','J. Smith'],
      ['Homa','McGreevy','Echavarria'],
      ['Saddier','Horschel','Thompson'],
      ['MacIntyre','Poston','Thomas'],
      ['Putnam','Bradley','Higgo'],
      ['Hoge','Dahmen','Cole'],
      ['Yu','Nakajima','Clanton'],
      ['Kuchar','Hodges','McCarty'],
      ['Rodgers','Stevens','Keefer'],
      ['Olesen','Bezuidenhout','Blanchet'],
    ]
  }
};
...
function getGolfSplitTickets(date, allPicks) {
  ...
  // If late has fewer than 10, fill from the next best picks not already used
  if (late.length < 10) {
    const usedIds = new Set([...early.map(l => l.id), ...late.map(l => l.id)]);
    for (const entry of entries) {
      if (late.length >= 10) break;
      if (!usedIds.has(entry[0])) late.push(toGolfLeg(entry));
    }
  }
